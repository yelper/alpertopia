---
title: French Toast
categories: [baked goods]
servings: 8
prep_time: 10
cook_time: 15
reference: https://tastesbetterfromscratch.com/classic-french-toast/
ingredients:
- slug: eggs
  amount: 4
  notes:
- slug: milk
  amount: 2/3 C
  notes:
- slug: flour
  amount: 1/4 C
  notes:
- slug: sugar
  amount: 1/4 C
  notes:
- slug: salt
  amount: 1/4 tsp
  notes:
- slug: cinnamon
  amount: 1 tsp
  notes:
- slug: vanilla extract
  amount: 1 tsp
  notes: 
- slug: bread
  amount: 8 slices
  notes:


---

1. Preheat griddle to 350 degrees F or or heat a skillet over medium heat.
2. In a blender, add the eggs, milk, flour, sugar, salt, cinnamon, and vanilla. Blend until smooth. If you prefer whisking by hand, start by mixing the flour and eggs together in a shallow dish, then whisk in the rest of the ingredients until combined.
3. Dip bread slices into the batter, dredging them well on both sides, and place on hot, greased griddle or skillet.
4. Cook for a few minutes, until the bottom of the breads starts to get golden brown. Flip and cook on the other side the same.
5. Remove to a plate. Serve warm with syrup and a sprinkle of powdered sugar.