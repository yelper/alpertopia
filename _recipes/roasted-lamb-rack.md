---
title: Roasted Rack of Lamb
categories: [meat dinner]
servings: 4
prep_time: 15
cook_time: 20
reference: https://www.allrecipes.com/recipe/45641/roasted-rack-of-lamb/
ingredients:
- slug: bread crumbs
  amount: 1/2 C
  notes:
- slug: garlic
  amount: 2 tbsp
  notes: minced
- slug: rosemary
  amount: 2 tbsp
  notes: chopped
- slug: salt
  amount: 1 tsp
  notes:
- slug: black pepper
  amount: 1/4 tsp
  notes:
- slug: olive oil
  amount: 2 tbsp
  notes:
- slug: 7 bone roack of lamb
  amount: 1
  notes:
- slug: salt
  amount: 1 tsp
  notes:
- slug: black pepper
  amount: 1 tsp
  notes:
- slug: olive oil
  amount: 2 tbsp
  notes:
- slug: dijon mustard
  amount: 1 tbsp
  notes: 

---



1. Gather all ingredients. Preheat the oven to 450 degrees F (230 degrees C). Move the oven rack to the center position.
2. Combine bread crumbs, garlic, rosemary, 1 teaspoon salt, and 1/4 teaspoon pepper in a small bowl; stir in 2 tablespoons olive oil to moisten the mixture. Set aside.
3. Season rack of lamb all over with 1 teaspoon salt and 1 teaspoon pepper.
A rack of lamb on a small baking sheet, seasoned with salt and pepper
4. Heat 2 tablespoons olive oil in a large heavy oven-proof skillet over high heat. Add lamb and sear on all sides, about 1 to 2 minutes: set lamb aside for a few minutes.
5. Brush lamb with mustard and roll in bread crumb mixture until evenly coated. Cover the ends of the bones with foil to prevent charring.
6. Arrange the breaded rack of lamb bone-side down in the same skillet. Roast in preheated oven for 12 to 18 minutes for medium; an instant-read thermometer inserted into the center should read at least 130 degrees F (54 degrees C), or continue to cook to desired doneness.
7. Remove lamb from the skillet and allow to rest for 5 to 7 minutes, loosely covered with foil, before carving between the ribs.